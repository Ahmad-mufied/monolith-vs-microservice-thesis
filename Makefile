...
help:
	...
	@echo "  make run-benchmark-arch-suite ARCHITECTURE=<monolith|microservices> SCALING_MODE=<fixed|hpa> EXPERIMENT_NAME=<name>"
...
run-benchmark-suite:
	...
	if [ "$(SCALING_MODE)" != "fixed" ]; then \
	  printf '%s\n' "ERROR: run-benchmark-suite only supports SCALING_MODE=fixed. Use make run-benchmark-arch-suite for a supplemental single-architecture HPA suite, or make run-benchmark-case for one-off HPA cases." >&2; \
	  exit 1; \
	fi; \
	...
	printf '  inter_case_delay         : %s\n' "$(INTER_CASE_DELAY)"; \
	printf '  architecture_switch_delay: %s\n' "$(ARCHITECTURE_SWITCH_DELAY)"; \
	printf '  auto_destroy_confirmed   : %s\n' "$(AUTO_DESTROY_CONFIRMED)"; \
	if [ -n "$(if $(filter command line environment,$(origin RUN_ID)),$(RUN_ID),)" ]; then \
	  printf '  run_id                   : %s\n' "$(if $(filter command line environment,$(origin RUN_ID)),$(RUN_ID),)"; \
	elif [ -n "$(if $(filter command line environment,$(origin EXPERIMENT_NAME)),$(EXPERIMENT_NAME),)" ]; then \
	  printf '%s\n' '  run_id                   : <auto-generated from EXPERIMENT_NAME and IMAGE_TAG>'; \
	  printf '  experiment_name          : %s\n' "$(if $(filter command line environment,$(origin EXPERIMENT_NAME)),$(EXPERIMENT_NAME),)"; \
	else \
	  printf '%s\n' '  run_id                   : <auto-generated from scaling mode and timestamp>'; \
	fi; \
	printf '  attempt                  : %s\n' "$(if $(filter command line environment,$(origin ATTEMPT)),$(ATTEMPT),)"; \
	printf '  image_tag                : %s\n' "$(IMAGE_TAG)"; \
	SCALING_MODE=$(SCALING_MODE) \
...
.PHONY: run-benchmark-arch-suite
run-benchmark-arch-suite:
	@printf '%s\n' '=== make run-benchmark-arch-suite ==='; \
	if [ "$(ARCHITECTURE)" = "monolith" ] && [ "$(SCALING_MODE)" = "hpa" ]; then \
	  printf '%s\n' "ERROR: run-benchmark-arch-suite does not support ARCHITECTURE=monolith with SCALING_MODE=hpa in the active benchmark model." >&2; \
	  exit 1; \
	fi; \
	printf '  architecture             : %s\n' "$(ARCHITECTURE)"; \
	printf '  scaling_mode             : %s\n' "$(SCALING_MODE)"; \
	printf '  k6_profile               : %s\n' "$(if $(filter command line environment,$(origin K6_PROFILE)),$(K6_PROFILE),<auto>)"; \
	printf '  test_duration            : %s\n' "$(TEST_DURATION)"; \
	if [ -n "$(strip $(SCENARIO_RPS_MATRIX))" ]; then \
	  printf '  scenario_rps_matrix      : %s\n' "$(SCENARIO_RPS_MATRIX)"; \
	  printf '%s\n' '  rps_levels               : <ignored because SCENARIO_RPS_MATRIX is set>'; \
	...
	if [ -n "$(if $(filter command line environment,$(origin RUN_ID)),$(RUN_ID),)" ]; then \
	  ...
	  printf '%s\n' '  run_id                   : <auto-generated from architecture, scaling mode, and timestamp>'; \
	fi; \
	printf '  attempt                  : %s\n' "$(if $(filter command line environment,$(origin ATTEMPT)),$(ATTEMPT),)"; \
	printf '  image_tag                : %s\n' "$(IMAGE_TAG)"; \
	...
	DOCKERHUB_NAMESPACE=$(DOCKERHUB_NAMESPACE) \
	IMAGE_TAG=$(IMAGE_TAG) \
	SEQUENTIAL_CASE_OVERHEAD_SECONDS=$(SEQUENTIAL_CASE_OVERHEAD_SECONDS) \
	SEQUENTIAL_REUSED_CASE_OVERHEAD_SECONDS=$(SEQUENTIAL_REUSED_CASE_OVERHEAD_SECONDS) \
	SEQUENTIAL_RETRY_BUFFER_SECONDS=$(SEQUENTIAL_RETRY_BUFFER_SECONDS) \
	bash scripts/operator-dispatch.sh run-benchmark-arch-suite
...
run-benchmark-suite-vultr:
	...
	if [ "$(SCALING_MODE)" != "fixed" ]; then \
	  printf '%s\n' "ERROR: run-benchmark-suite-vultr only supports SCALING_MODE=fixed. Use make run-benchmark-arch-suite EXECUTION_MODE=sequential ARCHITECTURE=microservices for a supplemental HPA suite, or make run-benchmark-case for one-off HPA cases." >&2; \
	  exit 1; \
	fi; \
	if [ -n "$(strip $(SCENARIO_RPS_MATRIX))" ]; then \
	  printf '  scenario_rps_matrix      : %s\n' "$(SCENARIO_RPS_MATRIX)"; \
...
.PHONY: run-benchmark-arch-suite-vultr
run-benchmark-arch-suite-vultr:
	@if [ "$(ARCHITECTURE)" = "monolith" ] && [ "$(SCALING_MODE)" = "hpa" ]; then \
	  printf '%s\n' "ERROR: run-benchmark-arch-suite-vultr does not support ARCHITECTURE=monolith with SCALING_MODE=hpa in the active benchmark model." >&2; \
	  exit 1; \
	fi
	CLOUD_PROVIDER=vultr \
	...
	SEQUENTIAL_REUSED_CASE_OVERHEAD_SECONDS=$(SEQUENTIAL_REUSED_CASE_OVERHEAD_SECONDS) \
	SEQUENTIAL_RETRY_BUFFER_SECONDS=$(SEQUENTIAL_RETRY_BUFFER_SECONDS) \
	bash scripts/run-benchmark-arch-suite.sh

.PHONY: run-benchmark-suite-sequential-vultr
run-benchmark-suite-sequential-vultr:
	@printf '%s\n' '=== make run-benchmark-suite-sequential-vultr ==='; \
	if [ "$(SCALING_MODE)" != "fixed" ]; then \
	  printf '%s\n' "ERROR: run-benchmark-suite-sequential-vultr only supports SCALING_MODE=fixed. Use make run-benchmark-arch-suite-vultr ARCHITECTURE=microservices for a supplemental HPA suite, or make run-benchmark-case for one-off HPA cases." >&2; \
	  exit 1; \
	fi; \
	if [ -n "$(strip $(SCENARIO_RPS_MATRIX))" ]; then \
	  printf '  scenario_rps_matrix      : %s\n' "$(SCENARIO_RPS_MATRIX)"; \
...
run-benchmark-suite-sequential:
	...
	if [ "$(SCALING_MODE)" != "fixed" ]; then \
	  printf '%s\n' "ERROR: run-benchmark-suite-sequential only supports SCALING_MODE=fixed. Use make run-benchmark-arch-suite ARCHITECTURE=microservices for a supplemental HPA suite, or make run-benchmark-case for one-off HPA cases." >&2; \
	  exit 1; \
	fi; \
	printf '  scaling_mode             : %s\n' "$(SCALING_MODE)"; \
	printf '  k6_profile               : %s\n' "$(if $(filter command line environment,$(origin K6_PROFILE)),$(K6_PROFILE),)"; \
	printf '  test_duration            : %s\n' "$(TEST_DURATION)"; \
	printf '  architecture_order       : %s\n' "$(ARCHITECTURE_ORDER)"; \
	if [ -n "$(strip $(SCENARIO_RPS_MATRIX))" ]; then \
	  printf '  scenario_rps_matrix      : %s\n' "$(SCENARIO_RPS_MATRIX)"; \
	  printf '%s\n' '  rps_levels               : <ignored because SCENARIO_RPS_MATRIX is set>'; \
	else \
