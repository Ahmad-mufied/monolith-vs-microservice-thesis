...
context="${SEQUENTIAL_CONTEXT:-monolith}"
output_env="${OCI_RESOURCE_BASELINE_ENV:-env/oci-resource-baseline.env}"
output_json="${OCI_RESOURCE_BASELINE_JSON:-env/oci-resource-baseline.json}"
safety_cpu_m="${OCI_RESOURCE_SAFETY_CPU_M:-0}"
safety_memory_mi="${OCI_RESOURCE_SAFETY_MEMORY_MI:-110}"
...
if [ "${OCI_RESOURCE_ROUNDING:-false}" = "true" ]; then
...
node_shape="${OCI_NODE_SHAPE:-unknown}"
region="${OCI_REGION:-unknown}"

cat > "$output_env" <<EOF
OCI_RESOURCE_BASELINE_PROVIDER=oci
OCI_REGION=${region}
OCI_NODE_SHAPE=${node_shape}
OCI_APP_CPU_QUOTA=${app_cpu_quota_m}m
OCI_APP_MEMORY_QUOTA=${app_memory_quota_mi}Mi
OCI_APP_NODE_COUNT=${node_count}
OCI_APP_ALLOCATABLE_CPU=${total_cpu_m}m
OCI_APP_ALLOCATABLE_MEMORY=${total_memory_mi}Mi
OCI_RESOURCE_SAFETY_CPU=${safety_cpu_m}m
OCI_RESOURCE_SAFETY_MEMORY=${safety_memory_mi}Mi
OCI_RESOURCE_BASELINE_MEASURED_AT_UTC=${measured_at}
EOF

jq -n \
  --arg context "$context" \
  --arg region "$region" \
  --arg node_shape "$node_shape" \
...
echo "Wrote OCI resource baseline:"
echo "  $output_env"
echo "  $output_json"
cat "$output_env"
