...
if [ "$ALLOW_NONSTANDARD_SCALING_PROFILE" != "true" ]; then
  case "$SCALING_MODE:$K6_PROFILE" in
    ...
    hpa:steady|hpa:ramp|hpa:smoke)
      echo "ERROR: SCALING_MODE=hpa requires K6_PROFILE=ramp-up for the standard autoscaling experiment. Set ALLOW_NONSTANDARD_SCALING_PROFILE=true only for a deliberate nonstandard experiment." >&2
      exit 1
      ;;
  esac
fi
