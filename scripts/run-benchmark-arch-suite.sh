#!/usr/bin/env bash
# Run a benchmark suite for one selected architecture on the sequential cluster.
set -euo pipefail
...
RENDER_ROOT="$ARCH_SUITE_WORKDIR/rendered"
MAX_INTER_CASE_DELAY=86400
declare -A CASE_RESULT_STATUS_CACHE=()

cleanup() {
  rm -rf "$ARCH_SUITE_WORKDIR"
}
trap cleanup EXIT
: > "$MATRIX_TSV"
: > "$CASES_JSONL"
...
validate_supported_scenario() {
  ...
  case "$scenario" in
    ...
    *)
      log_error "unsupported scenario '$scenario' (expected: login|create-transaction|enriched-transactions|concurrent-mixed-workload|mixed-workload|sync-items)"
      exit 1
      ;;
  esac
}
...
validate_rps_words() {
  ...
  if [ -z "${rps_words//[[:space:]]/}" ]; then
    log_error "RPS list must contain at least one positive integer"
    exit 1
  fi

  for target_rps in $rps_words; do
    if ! [[ "$target_rps" =~ ^[1-9][0-9]*$ ]]; then
      log_error "invalid RPS value '$target_rps' (expected: positive integer)"
      exit 1
    fi
  done
}
...
validate_arch_suite_inputs() {
  ...
  case "$EXECUTION_MODE" in
    sequential) ;;
    *)
      log_error "run-benchmark-arch-suite currently supports EXECUTION_MODE=sequential only"
      exit 1
      ;;
  esac
  ...
  case "$ARCHITECTURE" in
    ...
    *)
      log_error "unsupported ARCHITECTURE '$ARCHITECTURE' (expected: monolith|microservices)"
      exit 1
      ;;
  esac
  ...
  case "$SCALING_MODE" in
    fixed|hpa) ;;
    *)
      log_error "unsupported SCALING_MODE '$SCALING_MODE' (expected: fixed|hpa)"
      exit 1
      ;;
  esac

  if [ "$ARCHITECTURE" = "monolith" ] && [ "$SCALING_MODE" = "hpa" ]; then
    log_error "ARCHITECTURE=monolith with SCALING_MODE=hpa is not supported in the active benchmark model. Use SCALING_MODE=fixed for monolith, or ARCHITECTURE=microservices with SCALING_MODE=hpa for supplemental autoscaling runs."
    exit 1
  fi
  ...
  if [ -n "${SCENARIO_RPS_MATRIX//[[:space:]]/}" ]; then
    while IFS= read -r matrix_entry; do
      ...
      if [[ "$matrix_entry" != *:* ]]; then
        log_error "invalid SCENARIO_RPS_MATRIX entry '$matrix_entry' (expected: scenario:rps1,rps2,...)"
        exit 1
      fi
      ...
      validate_rps_words "$rps_words"
      matrix_entry_count=$((matrix_entry_count + 1))
    done < <(tr ';' '\n' <<<"$SCENARIO_RPS_MATRIX")

    if [ "$matrix_entry_count" -eq 0 ]; then
      log_error "SCENARIO_RPS_MATRIX must contain at least one scenario:rps entry"
      exit 1
    fi
  else
    if [ -z "${SCENARIOS//[[:space:]]/}" ]; then
      log_error "SCENARIOS must contain at least one scenario"
      exit 1
    fi
    ...
    validate_rps_words "$RPS_LEVELS"
  fi

  if ! [[ "$INTER_CASE_DELAY" =~ ^[0-9]+$ ]]; then
    log_error "invalid INTER_CASE_DELAY value '$INTER_CASE_DELAY' (expected: non-negative integer seconds, e.g. 300 for 5 minutes)"
    exit 1
  fi
  ...
  if integer_greater_than "$normalized_inter_case_delay" "$MAX_INTER_CASE_DELAY"; then
    log_error "invalid INTER_CASE_DELAY value '$INTER_CASE_DELAY' (maximum: ${MAX_INTER_CASE_DELAY} seconds)"
    exit 1
  fi
  ...
  case "$AUTO_DESTROY_CONFIRMED" in
    true|false) ;;
    *)
      log_error "invalid AUTO_DESTROY_CONFIRMED value '$AUTO_DESTROY_CONFIRMED' (expected: true|false)"
      exit 1
      ;;
  esac
  ...
  case "$SKIP_BENCHMARK_PREFLIGHT" in
    true|false) ;;
    *)
      log_error "invalid SKIP_BENCHMARK_PREFLIGHT value '$SKIP_BENCHMARK_PREFLIGHT' (expected: true|false)"
      exit 1
      ;;
  esac
...
log_arch_suite_configuration() {
  ...
  log_info "=== Architecture Benchmark Suite ==="
...
set_case_result_status_cache() {
  ...
  key="$(case_result_cache_key "$1" "$2")"
  CASE_RESULT_STATUS_CACHE["$key"]="$3"
}
...
prime_case_result_status_cache() {
  ...
  while IFS= read -r object_line; do
    ...
    if [[ "$relative_key" =~ ^([^/]+)/([^/]+)/([0-9]+)rps/([^/]+)/result-status\.json$ ]]; then
      ...
      if [ "$matched_architecture" = "$ARCHITECTURE" ] && [ "$matched_attempt" = "$ATTEMPT" ]; then
        set_case_result_status_cache "$matched_scenario" "$matched_rps" "present"
      fi
    fi
...
case_missing_in_s3() {
  ...
  key="$(case_result_cache_key "$scenario" "$target_rps")"
...
architecture_ready_for_resume() {
  ...
  scaling_mode_matches_live_architecture || return 1
  inactive_architecture_scaled_down || return 1
...
arch_suite_case_timing_json() {
  local s3_uri="$1"
  local case_started_at_utc="$2"
  local case_finished_at_utc="$3"
  ...
  attempt_timing_json="$(
    resolve_attempt_timing_json \
      ...
      "arch-suite ${ARCHITECTURE} ${s3_uri##*/}"
  )"
...
if [ -z "$RUN_ID" ]; then
  ...
  if [ -n "$EXPERIMENT_NAME" ]; then
    ...
    RUN_ID="${run_prefix}-${ARCHITECTURE}-${SCALING_MODE}-${EXPERIMENT_NAME}-${sanitized_image_tag}"
  else
    RUN_ID="${run_prefix}-${ARCHITECTURE}-${SCALING_MODE}-$(date +%Y%m%d-%H%M)"
  fi
fi

S3_RUN_URI="s3://${S3_BUCKET}/experiments/${RUN_ID}"
ARCH_SUITE_STARTED_AT_UTC="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
...
manifest_path="$ARCH_SUITE_WORKDIR/manifest.json"
jq -n \
...
log_info "Architecture suite finished successfully."
