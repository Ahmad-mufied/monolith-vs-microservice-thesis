...
func (r *PostgresRepository) Create(ctx context.Context, userID string, items []CreateItemRequest) (Transaction, error) {
	...
	if err != nil {
		return Transaction{}, internalError("beginning transaction", err)
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return Transaction{}, internalError("committing transaction", err)
	}
...
LIMIT $2 OFFSET $3`
	...
	if err != nil {
		return nil, internalError("listing own transactions", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&tx.ID, &tx.UserID, &tx.CreatedAt, &tx.UpdatedAt); err != nil {
			return nil, internalError("scanning transaction", err)
		}
		transactions = append(transactions, tx)
	}
	if err := rows.Err(); err != nil {
		return nil, internalError("iterating transactions", err)
	}
...
WHERE id = $1::uuid AND user_id = $2::uuid`
	...
	if err := r.db.QueryRow(ctx, query, transactionID, userID).Scan(&tx.ID, &tx.UserID, &tx.CreatedAt, &tx.UpdatedAt); err != nil {
		...
		return Transaction{}, internalError("getting transaction", err)
	}
...
ORDER BY t.created_at DESC, t.id DESC, i.id`
	...
	if err != nil {
		return nil, internalError("listing enriched transactions", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			return nil, internalError("scanning enriched transaction", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, internalError("iterating enriched transactions", err)
	}
...
ORDER BY item_id`
	...
	if err != nil {
		return nil, internalError("listing transaction items", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ItemID, &item.Amount); err != nil {
			return nil, internalError("scanning transaction item", err)
		}
		items = append(items, item)
	}
	if err := rows.Err(); err != nil {
		return nil, internalError("iterating transaction items", err)
	}
	return items, nil
}
...
func validateItem(ctx context.Context, tx pgx.Tx, item CreateItemRequest) error {
	...
	if err := tx.QueryRow(ctx, `SELECT available_amount FROM items WHERE id = $1::uuid AND deleted_at IS NULL`, item.ItemID).Scan(&availableAmount); err != nil {
		...
		return internalError("validating item", err)
	}
...
func insertTransaction(ctx context.Context, tx pgx.Tx, userID string) (Transaction, error) {
	...
	if err := tx.QueryRow(ctx, query, userID).Scan(&transaction.ID, &transaction.UserID, &transaction.CreatedAt, &transaction.UpdatedAt); err != nil {
		...
		return Transaction{}, internalError("inserting transaction", err)
	}
	return transaction, nil
}
...
VALUES ($1::uuid, $2::uuid, $3)`
	if _, err := tx.Exec(ctx, query, transactionID, item.ItemID, item.Amount); err != nil {
		return internalError("inserting transaction item", err)
	}
	return nil
}
