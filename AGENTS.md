...
For benchmark users and items, seed scripts may use deterministic UUIDs so k6
and other tooling can rely on stable identities without reading them back from
the database first.
...
Seed scripts must still capture generated IDs when the inserted records do not
use predetermined IDs, such as generated transaction rows for enrichment data.
