...
returns bounded non-success responses, a derived `status-summary.json` from
`raw.json.gz` should be used to separate successful `2xx` latency and goodput
from overload/error responses before drawing conclusions from latency
percentiles.
