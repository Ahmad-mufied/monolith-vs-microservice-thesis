...
│  4. make eks-update-manifests      Patch image URI in YAML    │
...
The reason `eks-update-manifests` runs immediately after push is that
the manifests must point to the same `IMAGE_TAG` that was just pushed.
Doing this step later opens a window where the operator could deploy a
different commit's manifests against ECR, causing reproducibility issues
in `metadata.json`.
...
Approximate cost per hour at `ap-southeast-1` on-demand pricing. Numbers
are illustrative — the live AWS Pricing Calculator should be used for
budget planning.
...
RPS levels, destroy — takes 3–4 hours and consumes roughly $4–$5.
